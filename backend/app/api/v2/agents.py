...
@router.get("/summary")
async def agents_summary(db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
...
async def create_evaluation(body: EvaluationCreate, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    # Validate run exists
    run = (await db.execute(
